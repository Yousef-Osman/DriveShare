...
public static class LinqExtensions
{
    ...
    public static IQueryable<T> WhereDynamic<T>(this IQueryable<T> sourceList, string query)
    {
        ...
        try
        {
            var properties = typeof(T).GetProperties().Where(x => x.CanRead && x.CanWrite && !x.GetGetMethod().IsVirtual);

            //Expression
            sourceList = sourceList.Where(c => properties.Any(p => p.GetValue(c) != null && p.GetValue(c).ToString()
